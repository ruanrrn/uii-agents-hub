---
name: rib-detection-skill
description: "Use when the user asks to run a RIB / rib fracture imaging analysis — e.g. '对这个肋骨CT做rib检测', '帮我分析肋骨骨折影像', '肋骨影像分析', 'rib fracture analysis'. Covers installing/verifying the imaging-detection MCP dependency, normalizing user input (remote 7z URL or local .7z file), submitting via create_imaging_task, polling via get_imaging_task, and presenting the diagnostic report (findings, conclusions, emergencies) plus viewer URL. Supports stdio MCP (default) and optional HTTP MCP access."
---

# RIB Detection Skill

对 DICOM 影像做**肋骨骨折（RIB）** AI 检测。将用户输入归一化为 `{ type: "rib", storageUrl }`，通过 `imaging-detection-mcp` 的两个工具完成分析并展示诊断报告。
...
如果用户意图不明确或同时提到脑卒中/脑出血，询问用户确认是 RIB 还是 ICH。
...
5. 调用 `create_imaging_task`：
   ```json
   { "type": "rib", "storageUrl": "<storageUrl>" }
   ```
