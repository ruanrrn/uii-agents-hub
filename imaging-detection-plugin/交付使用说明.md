...
本文档面向交付、部署、首次使用与维护人员，覆盖 `imaging-detection-mcp`（MCP server）与影像检测 skill（Agent 技能）两部分：既包含安装、配置、验收等使用说明，也包含架构、模块职责、工具与数据流等设计说明。各 skill 的 `SKILL.md` 是给 Agent 读取的技能说明；本文档是给人阅读的使用与设计文档。

> **重要变更**：原 `imaging-detection-skill` 已拆分为两个专用 skill：
> - `ich-detection-skill`：卒中 / 脑出血（ICH）影像分析
> - `rib-detection-skill`：肋骨骨折（RIB）影像分析
>
> 原 `imaging-detection-skill` 保留为索引/重定向，仅在用户意图不明确时引导选择。
> 两个专用 skill 各自包含独立的 `SKILL.md`、`references/` 参考文档和 `scripts/` 脚本。

## 职责边界（重要）

影像检测 skill 的职责**仅限于指导 Agent 如何编排流程来调用 MCP 工具**：即在已经拿到「检测类型 + DICOM 7z 可下载 URL」之后，如何归一化输入、按 `create_imaging_task` → `get_imaging_task` 的顺序调用、并格式化输出诊断报告。
...
- 再把得到的 `storageUrl` 连同检测类型交给对应 skill，按本文档的流程完成影像检测。

换言之，skill 是「拿到影像 URL 之后」的检测编排层；「如何根据患者信息拿到影像 URL」不在 skill 范围内，由上层 Agent 自行补齐。

## 交付内容

```text
imaging-detection-plugin/
  imaging-detection-skill/          # 索引/重定向（保留兼容）
    SKILL.md
    scripts/
      serve-dicom-7z.mjs
    交付使用说明.md                  # 本文档
  ich-detection-skill/              # ICH 专用 skill
    SKILL.md
    references/
      mcp-setup.md                  # MCP 安装与配置（按需加载）
      mcp-tools.md                  # 工具字段与返回结构（按需加载）
      error-guidance.md             # 错误排查（按需加载）
    scripts/
      serve-dicom-7z.mjs
  rib-detection-skill/              # RIB 专用 skill
    SKILL.md
    references/
      mcp-setup.md
      mcp-tools.md
...
- `SKILL.md`：Agent 技能定义，描述该检测类型的分析流程、前置检查、输入通道和响应格式。精简版，约 120 行。
- `references/`：参考文档，供 Agent 按需加载（安装配置、工具详情、错误排查），避免 SKILL.md 过长。
- `scripts/serve-dicom-7z.mjs`：本地 DICOM `.7z` 文件临时 HTTP 发布脚本。
- `交付使用说明.md`：本文档。

## 功能范围

两个专用 skill 分别把用户输入转换为对应类型的任务提交：
...
支持两类影像分析，分别由对应 skill 处理：

| 用户输入                              | MCP 参数 | 专用 Skill              |
| ------------------------------------- | -------- | ----------------------- |
| `ICH`、脑卒中、卒中、脑出血、颅内出血 | `ich`    | `ich-detection-skill`   |
| `RIB`、肋骨、肋骨骨折                 | `rib`    | `rib-detection-skill`   |
...
根据需要安装对应的专用 skill 目录到目标 Agent 支持的 skills 目录中：
...
也可以同时安装两个 skill，Agent 会根据用户意图自动选择。如需保留兼容性，可同时安装原 `imaging-detection-skill`（索引/重定向）。
